  ...
  return (
    <>
        ...
        <div className="max-w-[1440px] mx-auto px-6 lg:px-10 flex items-center justify-between">
          ...
          <div className={`hidden lg:flex absolute left-1/2 -translate-x-1/2 items-center gap-8 ${language === 'ar' ? 'font-cairo' : 'font-sora'}`}>
            {navLinks.map((link, index) => (
              <a 
                key={index} 
                href={link.href} 
                className={`relative text-[14px] font-medium no-underline transition-colors duration-300 group ${isScrolled ? 'text-slate-300 hover:text-white' : 'text-slate-800/75 hover:text-slate-950'}`}
              >
                {link.name}
                <span className="absolute left-1/2 -bottom-1.5 w-0 h-[2px] bg-gradient-to-r from-[#07B98F] to-[#3ECEB0] transition-all duration-300 group-hover:w-full group-hover:left-0 rounded-full"></span>
              </a>
            ))}
          </div>
          ...
          <div className="flex items-center gap-4 z-50 relative">
            <button
              ...
              className={`flex items-center gap-2 py-2 px-4 rounded-full transition-all ${isScrolled ? 'bg-white/10 border border-white/10 text-white hover:bg-white/20' : 'bg-slate-900/5 border border-slate-900/10 text-slate-800 hover:text-slate-950 hover:bg-slate-900/10'}`}
            >
              <FaGlobe />
            ...
            <a 
              href="#contact" 
              className={`relative hidden lg:flex items-center justify-center py-2.5 px-8 rounded-full overflow-hidden group transition-all duration-500 hover:border-transparent hover:shadow-[0_0_24px_rgba(7,185,143,0.3)] hover:-translate-y-0.5 ${isScrolled ? 'bg-white/10 border border-white/10 text-white' : 'bg-slate-900/5 border border-slate-900/10 text-slate-800'} ${language === 'ar' ? 'font-cairo' : 'font-sora'}`}
            >
              <span className="absolute inset-0 w-full h-full bg-gradient-to-r from-[#07B98F] to-[#3ECEB0] opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-out"></span>
              <span className={`relative text-[14px] font-bold transition-colors duration-300 z-10 group-hover:text-white ${isScrolled ? 'text-white' : 'text-slate-800'}`}>{t('nav.book')}</span>
            </a>
            <button 
              className={`hamburger flex lg:hidden flex-col justify-center items-center gap-1.5 transition-colors duration-300 cursor-pointer p-2.5 rounded-full ${isScrolled ? 'bg-white/10 hover:bg-white/20 border border-white/10 text-white' : 'bg-slate-900/5 hover:bg-slate-900/10 border border-slate-900/10 text-slate-800'}`} 
      ...
      <div className={`fixed top-0 ${language === 'ar' ? 'left-0 border-r' : 'right-0 border-l'} bottom-0 w-4/5 max-w-[320px] bg-white border-black/5 shadow-[-10px_0_30px_rgba(0,0,0,0.5)] z-[1002] flex flex-col p-[30px] gap-6 transition-transform duration-400 ease-[cubic-bezier(0.22,1,0.36,1)] ${isOpen ? 'translate-x-0' : (language === 'ar' ? '-translate-x-full' : 'translate-x-full')}`}>
        ...
        <a href="#contact" onClick={() => setIsOpen(false)} className={`mt-5 text-[16px] p-4 bg-gradient-to-br from-[#07B98F] to-[#3ECEB0] text-white rounded-xl font-bold no-underline text-center shadow-[0_8px_20px_rgba(7,185,143,0.15)] ${language === 'ar' ? 'font-cairo' : 'font-sora'}`}>
          {t('nav.book')}
        </a>
      </div>
    </>
