...
// Site-wide interaction script. Bundled (served via /_next) instead of being
// fetched from /public, which 404s on the live deployment.
export default function SiteScripts() {
  useEffect(() => {
    ...
    try {
      const canvas: any = document.getElementById('particles-canvas');
      if (canvas) {
    ...
    try {
      const intWrap: any = document.querySelector('.int-scroll-wrap');
      ...
      if (intWrap && intSteps.length) {
        const updateIntStep = () => {
          ...
          const progress = Math.max(0, Math.min(0.9999, total > 0 ? scrolled / total : 0));
